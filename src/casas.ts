...
class Casa {
  ...
  public comprar(compradores: Persona[]): void {
    ...
    for (const comprador of compradores) {
      totalDinero += comprador.dineroDisponible;
    }

    if (totalDinero >= this.precio) {
      console.log("Casa comprada");
    } else {
      console.error("ERROR: Los compradores no tienen suficiente dinero para comprar esta casa.");
    }
  }
}
